<template>
  <div class="min-h-screen bg-slate-50">
    <div class="w-full px-4 sm:px-6 lg:px-8 py-6">
      <!-- Page Header -->
      <div class="mb-8">
        <h1 class="text-4xl font-extrabold text-slate-900 mb-2">Create New Order</h1>
        <p class="text-slate-600 mt-1">Configure your production order</p>
      </div>
      ...
      <div class="flex gap-8">
        ...
        <div class="w-80 space-y-6">
          ...
          <div class="bg-white rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-slate-900 mb-4">Progress</h3>
            <div class="space-y-3">
              <button
              ...
              >
                ...
                <div>
                  ...
                  <div class="text-xs text-slate-500">Basic information & scheduling</div>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  ...
                  <div class="text-xs text-slate-500">Product specifications</div>
                </div>
              </button>
            </div>
          </div>
          ...
          <div v-if="orderData.name || orderData.clientId || orderData.sorts.length > 0" class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-2 text-sm">
              <div v-if="orderData.name">
                ...
                <div class="font-medium text-slate-900">{{ orderData.name }}</div>
              </div>
              <div v-if="orderData.clientId">
                <span class="text-slate-600">Client:</span>
                <div class="font-medium text-slate-900">{{ getClientName(orderData.clientId) }}</div>
              </div>
              <div v-if="orderData.sorts.length > 0">
                <span class="text-slate-600">Sorts:</span>
                <div class="font-medium text-slate-900">{{ orderData.sorts.length }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg shadow p-6">
            <div class="space-y-3">
              ...
              <button class="w-full px-4 py-2 rounded-lg text-sm font-medium bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 transition-colors">
                Save as Draft
              </button>
            </div>
          </div>
        ...
        <div v-if="activeSection === 'details'" class="bg-white rounded-lg shadow p-8">
          <div class="mb-6">
            <h2 class="text-2xl font-semibold text-slate-900 mb-2">Order Details</h2>
            <p class="text-slate-600">Configure the basic information for your production order</p>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
              ...
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">Order Name *</label>
                <input 
                  v-model="orderData.name"
                  type="text" 
                  class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                  placeholder="Enter order name"
                />
              </div>
              
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">Custom Order ID</label>
                <input 
            ...
            <!-- Client Information -->
            <div class="space-y-6">
              <h3 class="text-lg font-medium text-slate-900">Client Information</h3>
              ...
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">Project Reference</label>
                <input 
          ...
          <!-- Scheduling -->
          <div class="mt-8">
            <h3 class="text-lg font-medium text-slate-900 mb-6">Scheduling</h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">Required Date *</label>
                <input 
              ...
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">Preferred Line</label>
                <select 
          ...
          <!-- Volume and Species -->
          <div class="mt-8">
            <h3 class="text-lg font-medium text-slate-900 mb-6">Volume and Species</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-2">Expected Volume</label>
                <div class="flex space-x-2">
                  <input 
                    v-model="orderData.expectedVolume"
                    type="number" 
                    class="flex-1 px-4 py-3 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                    placeholder="0"
                  />
                  <select 
                    v-model="orderData.volumeUnit"
                    class="px-4 py-3 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                  >
        ...
        <div v-if="activeSection === 'sorts'" class="bg-white rounded-lg shadow p-8">
          <div class="flex items-center justify-between mb-6">
            <div>
              <h2 class="text-2xl font-semibold text-slate-900 mb-2">Define Sorts</h2>
              <p class="text-slate-600">Configure board geometry, grades, and color sorting for each sort</p>
            </div>
          </div>

          <!-- Added Sorts List -->
          <div v-if="orderData.sorts.length > 0" class="mb-8">
            <h3 class="text-lg font-medium text-slate-900 mb-4">Added Sorts ({{ orderData.sorts.length }})</h3>
            <div class="space-y-4">
              <div v-for="(sort, index) in orderData.sorts" :key="index" 
                ...
                <div class="flex items-center justify-between mb-4">
                  <h4 class="font-medium text-slate-900 text-lg">{{ sort.name }}</h4>
                  <button 
          ...
          <div class="bg-slate-50 rounded-lg p-6 border border-slate-200">
            <h3 class="text-lg font-semibold text-slate-900 mb-6">Create New Sort</h3>
            
            <!-- Basic Sort Info -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Sort Name *</label>
                <input 
              ...
              <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Target Volume *</label>
                <div class="flex space-x-2">
                  <input 
                  ...
                  <select 
                    ...
                    class="px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                  >
                    <option value="m3">m³</option>
                    <option value="bf">Board Feet</option>
                    <option value="pieces">Pieces</option>
            ...
            <div class="mb-6">
              <label class="block text-sm font-medium text-slate-700 mb-2">Special Requirements</label>
              <input 
